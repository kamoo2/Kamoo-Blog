---
title: z-index 값이 더 높은데 왜 ?!?
description: 혹시 z-index로 요소의 z축을 설정해준 경험이 있으신가요 ? 대부분 해보셨을텐데 z-index 값을 높게 설정했지만 원하는대로 작동하지 않는(위로 올라오지 않는) 경험을 해보셨을 것 같아요! 그래서 z-index는 어떤 원리로 동작하는지에 대해서 공부해보고자 합니다 👍
tags:
  - Css
createdAt: 2022-08-17 21:00:00
---
...
- position 속성이 static 이 아닌 요소는 무조건 position 속성이 static인 속성 위로 올라온다.
...
브라우저는 z-index 가 낮은 요소를 먼저 그리고, z-index 가 높은 요소를 나중에 그리기 때문에 요소가 겹처 있을 경우z-index 속성 값이 큰 요소가 z-index 속성값이 작은 요소의 일부를 가리거나 전체를 덮을 수 있다.
